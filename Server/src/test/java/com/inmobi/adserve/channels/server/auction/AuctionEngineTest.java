...
public class AuctionEngineTest {
  ...
  public void setUp() throws IOException {
    ...
    expect(repositoryHelper.queryCreativeRepository(EasyMock.isA(String.class), EasyMock.isA(String.class))).andReturn(
        null).anyTimes();
  ...
  private ChannelSegment setBidder(final String advId, final String channelId, final String externalSiteKey,
    ...
    final ChannelSegmentEntity channelSegmentEntity1 =
        new ChannelSegmentEntity(ChannelSegmentFilterApplierTest.getChannelSegmentEntityBuilder(advId, "adgroupId",
            "adId", channelId, 1, rcList, tags, true, true, externalSiteKey, modified_on, "campaignId", slotIds, 1,
            true, "pricingModel", siteRatings, 1, null, false, false, false, false, false, false, false, false, false,
            false, null, null, 0.0d, null, null, false, new HashSet<String>(), 0));
    ...
    final AdNetworkInterface mockAdnetworkInterface = createMock(RtbAdNetwork.class);
  ...
  public Object[][] paramDataProviderWith3Bidders() {
    return new Object[][] {
        ...
        {"testTwoSameBidsHigherThanFloorPrice", .70d, "A", 1d, 120l, "B", .80d, 100l, "C", .80d, 150l, .80d, "A", 1d},

        // 10. 2 same second bids, with same latency, higher than floor price
        {"testTwoSameSecondBidsWithSameLatencyHigherThanFloorPrice", .70d, "A", 1d, 120l, "B", .80d, 100l, "C", .80d,
            100l, .80d, "A", 1d},

        // 11. 2 same second bids, lower than floor price
        {"testTwoSameSecondBidsLowerThanFloorPrice", .70d, "A", 1d, 120l, "B", .60d, 100l, "C", .60d, 150l, .70d, "A",
            1d},

        // 12. 2 same second bids, lower than 90% of Highest bid but more than floor price
        {"testTwoSameSecondBidsLowerThan90PercentOfHighestBidButMoreThanFloorPrice", .70d, "A", 1d, 120l, "B", .85d,
            100l, "C", .85d, 150l, .85d, "A", 1d},

        // 13. 2 same second bids, lower than 90% of Highest bid and lower than floor price
        {"testTwoSameSecondBidsLowerThan90PercentOfHighestBidAndLowerThanFloorPrice", .70d, "A", 1d, 120l, "B", .65d,
            100l, "C", .65d, 150l, .70d, "A", 1d},

        // 14. 2 same second bids, higher than 90% of Highest bid but more than floor price
        {"testTwoSameSecondBidsHigherThan90PercentOfHighestBidButMoreThanFloorPrice", .70d, "A", 1d, 120l, "B", .95d,
            100l, "C", .95d, 150l, .95d, "A", 1d},

        // 15. 2 same second bids, higher than 90% of Highest bid and lower than floor price
        {"testTwoSameSecondBidsHigherThan90PercentOfHighestBidButLowerThanFloorPrice", .98d, "A", 1d, 120l, "B", .95d,
            100l, "C", .95d, 150l, .98d, "A", 1d},

        // 18. First and Second bid with .01 difference. He will be charged same Bid he auctioned
        {"testFirstAndSecondBidWith01difference", .70d, "A", 1d, 100l, "B", .99d, 100l, "C", .70d, 100l, 0.99d, "A", 1d},

        // 19. 2nd price same as floor price
        {"testSecondPriceSameAsFloorPrice", .70d, "A", 1d, 100l, "B", .70d, 100l, "C", .70d, 100l, .70d, "A", 1d},

        // 20. 2nd price same as floor price and 90% price
        {"testSecondPriceSameAsFloorPriceAnd90PercentPrice", .90d, "A", 1d, 100l, "B", .90d, 100l, "C", .70d, 100l,
            .90d, "A", 1d}
  ...
  public Object[][] paramDataProviderWith4Bidders() {
    return new Object[][] {
        ...
        {"testBidHigherThanFloorPriceAndSecondPrice", .70d, "A", 1d, 50l, "B", .90d, 100l, "C", .80d, 150l, "D", .70d,
            100l, .90d, "A", 1d},

        // 2. same first bid price
        {"testSameFirstBidPrice", .70d, "A", 1d, 100l, "B", 1d, 75l, "C", 1d, 50l, "D", .70d, 100l, .70d, "C", 1d},

        // 3. same second bid price
        {"testSameSecondBidPrice", .70d, "A", 1d, 100l, "B", .80d, 50l, "C", .80d, 150l, "D", .70d, 100l, .80d, "A", 1d},

        // 4. 2 win bid with same latency
        {"testTwoWinBidsWithSameLantency", .70d, "A", 1d, 100l, "B", 1d, 100l, "C", .80d, 150l, "D", .70d, 100l, .80d,
            "A", 1d},

        // 6. lower than floor price, the second bid onwards
        {"testBidLowerThanFloorPriceFromSecondBidOnwards", .70d, "A", 1d, 100l, "B", .40d, 100l, "C", .20d, 150l, "D",
            .45d, 100l, .70d, "A", 1d},

        // 8. all same bids, then latency is considered
        {"testAllSameBidsWithDifferentLatency", .70d, "A", 1d, 120l, "B", 1d, 100l, "C", 1d, 150l, "D", 1d, 70l, .70d,
            "D", 1d},

        // 16. 2 same first bid with 2nd Highest bid with 0.01 difference
        {"testTwoSameFirstBidWithSecondHighestBidWith01difference", .70d, "A", 1d, 100l, "B", 1d, 100l, "C", .99d,
            150l, "D", .70d, 100l, 0.99d, "A", 1d},

        // 17. First and Second bid with .01 difference and double 2nd entry. He will charged sameBid he
        // auctioned
        {"testFirstAndSecondBidWith01differenceAndDoubleSecondEntry", .70d, "A", 1d, 100l, "B", .99d, 100l, "C", .99d,
            150l, "D", .70, 100l, 0.99d, "A", 1d},

        // 21. 2 Highest price, and 2 same Second price
        {"testSecondHighestPriceAndTwoSameSecondPrice", .70d, "A", 1d, 100l, "B", 1d, 100l, "C", .90d, 150l, "D", .90d,
            100l, .90d, "A", 1d},

        // 22. 2 Highest price, and 2 same Second price, lower than floor price
        {"testTwoSecondHighestPriceAndTwoSameSecondPriceButLowerThanFloorPrice", .70d, "A", 1d, 100l, "B", 1d, 100l,
            "C", .60d, 150l, "D", .60d, 100l, .70d, "A", 1d},

        // 23. 2nd price is 0.1 above floor price
        {"testSecondPriceIs1AboveFloorPrice", .70d, "A", .71d, 100l, "B", 1d, 100l, "C", .60d, 150l, "D", .60d, 100l,
            .71d, "B", 1d},

        // 24. 2nd price is 0.1 below floor price. this bid will get directly cut off at floor price filter
        {"testSecondPriceIs1BelowFloorPrice", .70d, "A", 1d, 100l, "B", .69d, 100l, "C", .60d, 150l, "D", .60d, 100l,
            .70d, "A", 1d}
  ...
  public void testOnlyOneBid() {
    ...
    final Double expectedSecondPriceVal = .70d;
  ...
  public void testAllLowerThanFloorPrice() {
    ...
    final Double bidFloorInput = .70d;
    final Double bidInputVal1 = .50d;
    final Long latencyInputVal1 = 100l;
    final Double bidInputVal2 = .60d;
    final Long latencyInputVal2 = 100l;
    final Double bidInputVal3 = .40d;
    final Long latencyInputVal3 = 150l;
    final Double bidInputVal4 = .50d;
    final Long latencyInputVal4 = 100l;
    final Double expectedAuctionEngineResponse = null;
