...
final class DefaultChannelHandlerContext extends DefaultAttributeMap implements ChannelHandlerContext {
    ...
    public ChannelHandlerContext fireExceptionCaught(final Throwable cause) {
        ...
        if (executor.inEventLoop()) {
            ...
            try {
              InspectorStats.incrementStatCount("netty", InspectorStrings.NUMBER_OF_TIMES_READ_EXCEPTION_CALLED);

              final long startTime = System.currentTimeMillis();
                executor.execute(new OneTimeTask() {
                    @Override
                    public void run() {
                      InspectorStats.incrementStatCount("netty", InspectorStrings.NUMBER_OF_TIMES_READ_EXCEPTION_EXECUTED);
                      long latency = System.currentTimeMillis() - startTime;
                      InspectorStats.updateYammerTimerStats("netty", InspectorStrings.LATENCY_FOR_READING, latency);
    ...
    private void write(Object msg, boolean flush, ChannelPromise promise) {
        ...
        if (executor.inEventLoop()) {
            ...
            Runnable task;
            if (flush) {
                InspectorStats.incrementStatCount("netty", InspectorStrings.NUMBER_OF_TIMES_WRITE_CALLED);
                task = WriteAndFlushTask.newInstance(next, msg, size, promise, System.currentTimeMillis());
            }  else {
    ...
    abstract static class AbstractWriteTask extends OneTimeTask {
        ...
        protected static void init(AbstractWriteTask task, DefaultChannelHandlerContext ctx,
                                   Object msg, int size, ChannelPromise promise, long startWriteTime) {
            task.ctx = ctx;
            task.msg = msg;
            task.promise = promise;
            task.size = size;
            task.startWriteTime = startWriteTime;
        }
        ...
        public final void run() {
            try {
              if(startWriteTime != 0){
                long latency = System.currentTimeMillis() - startWriteTime;
                InspectorStats.incrementStatCount("netty", InspectorStrings.NUMBER_OF_TIMES_WRITE_EXECUTED);
                InspectorStats.updateYammerTimerStats("netty", InspectorStrings.LATENCY_FOR_WRITE, latency);
              }
                if (size > 0) {
    ...
    static final class WriteTask extends AbstractWriteTask implements SingleThreadEventLoop.NonWakeupRunnable {
        ...
        private static WriteTask newInstance(
            ...
            init(task, ctx, msg, size, promise, 0);
            return task;
        }
    ...
    static final class WriteAndFlushTask extends AbstractWriteTask {
        ...
        private static WriteAndFlushTask newInstance(
                DefaultChannelHandlerContext ctx, Object msg, int size, ChannelPromise promise, long startTime) {
            WriteAndFlushTask task = RECYCLER.get();
            init(task, ctx, msg, size, promise, startTime);
            return task;
        }
