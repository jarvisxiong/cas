...
public class SegmentFactory {
    ...
    public static AdNetworkInterface getChannel(String advertiserId, String channelId, Configuration config,
        ...
        if (isRtbEnabled) {
            for (String partnerName : rtbAdaptersNames) {
                ...
                if ((advertiserId.equalsIgnoreCase(advertiserIdString))
                        ...
                        && (config.getString(partnerName + ".status").equalsIgnoreCase("on") && config.getBoolean(
        ...
        if ((advertiserId.equals(config.getString("atnt.advertiserId")))
        ...
        else if ((advertiserId.equals(config.getString("mobilecommerce.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("mobilecommerce"))
        ...
        else if ((advertiserId.equals(config.getString("drawbridge.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("drawbridge"))
        ...
        else if ((advertiserId.equals(config.getString("mullahmedia.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("mullahmedia"))
        ...
        else if ((advertiserId.equals(config.getString("webmoblink.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("webmoblink"))
        ...
        else if ((advertiserId.equals(config.getString("siquis.advertiserId")))
        ...
        else if ((advertiserId.equals(config.getString("huntmads.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("huntmads"))
                && (config.getString("huntmads.status").equals("on"))) {
            return new DCPHuntmadsAdNetwork(logger, config, clientBootstrap, base, serverEvent);
        }
        else if ((advertiserId.equals(config.getString("httpool.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("httpool"))
        ...
        else if ((advertiserId.equals(config.getString("lomark.advertiserId")))
        ...
        else if ((advertiserId.equals(config.getString("pubmatic.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("pubmatic"))
                && (config.getString("pubmatic.status").equals("on"))) {
            return new DCPPubmaticAdNetwork(logger, config, clientBootstrap, base, serverEvent);
        }
        else if ((advertiserId.equals(config.getString("nexage.advertiserId")))
        ...
        else if ((advertiserId.equals(config.getString("adelphic.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("adelphic"))
                && (config.getString("adelphic.status").equals("on"))) {
            return new DCPAdelphicAdNetwork(logger, config, clientBootstrap, base, serverEvent);
        }
        else if ((advertiserId.equals(config.getString("mmpremium.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("mmpremium"))
        ...
        else if ((advertiserId.equals(config.getString("widerplanet.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("widerplanet"))
                && (config.getString("widerplanet.status").equals("on"))) {
            return new DCPWiderPlanetAdnetwork(logger, config, clientBootstrap, base, serverEvent);
        }
            ...
            return new DCPLoganAdnetwork(logger, config, clientBootstrap, base, serverEvent);
        }
        else if ((advertiserId.equals(config.getString("madnet.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("madnet"))
                && (config.getString("madnet.status").equals("on"))) {
            return new DCPMadNetAdNetwork(logger, config, clientBootstrap, base, serverEvent);
        }
        else if ((advertiserId.equals(config.getString("appier.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("appier"))
                && (config.getString("appier.status").equals("on"))) {
            return new DCPAppierAdNetwork(logger, config, clientBootstrap, base, serverEvent);
        }
        else if ((advertiserId.equals(config.getString("definiti.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("definiti"))
        ...
        else if ((advertiserId.equals(config.getString("paypal.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("paypal"))
                && (config.getString("paypal.status").equals("on"))) {
            return new DCPPayPalAdNetwork(logger, config, clientBootstrap, base, serverEvent);
        }
        else if ((advertiserId.equals(config.getString("selectmedia.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("selectmedia"))
                && (config.getString("selectmedia.status").equals("on"))) {
            DCPAjillionAdnetwork adaptor = new DCPAjillionAdnetwork(logger, config, clientBootstrap, base, serverEvent);
            adaptor.setName("selectmedia");
            return adaptor;
        }
        else if ((advertiserId.equals(config.getString("amobee.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("amobee"))
                && (config.getString("amobee.status").equals("on"))) {
            DCPAmobeeAdnetwork adaptor = new DCPAmobeeAdnetwork(logger, config, clientBootstrap, base, serverEvent);
            return adaptor;
        }
        else if ((advertiserId.equals(config.getString("ybrant.advertiserId")))
        ...
        else if ((advertiserId.equals(config.getString("mable.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("mable"))
                && (config.getString("mable.status").equals("on"))) {
            DCPMableAdnetwork adaptor = new DCPMableAdnetwork(logger, config, clientBootstrap, base, serverEvent);
            return adaptor;
        }
        else if ((advertiserId.equals(config.getString("placeiq.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("placeiq"))
                && (config.getString("placeiq.status").equals("on"))) {
            DCPPlaceIQAdnetwork adaptor = new DCPPlaceIQAdnetwork(logger, config, clientBootstrap, base, serverEvent);
            return adaptor;
        }
        else if ((advertiserId.equals(config.getString("wapstart.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("wapstart"))
                && (config.getString("wapstart.status").equals("on"))) {
            DCPWapStartAdNetwork adaptor = new DCPWapStartAdNetwork(logger, config, clientBootstrap, base, serverEvent);
            return adaptor;
        }
        else if ((advertiserId.equals(config.getString("crimzo.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("crimzo"))
                && (config.getString("crimzo.status").equals("on"))) {
            DCPAjillionAdnetwork adaptor = new DCPAjillionAdnetwork(logger, config, clientBootstrap, base, serverEvent);
            adaptor.setName("crimzo");
            return adaptor;
        }
        else if ((advertiserId.equals(config.getString("merimedia.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("merimedia"))
                && (config.getString("merimedia.status").equals("on"))) {
            DCPAppNexusAdnetwork adaptor = new DCPAppNexusAdnetwork(logger, config, clientBootstrap, base, serverEvent);
            adaptor.setName("merimedia");
            return adaptor;
        }
        else if ((advertiserId.equals(config.getString("inneractive.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("inneractive"))
                && (config.getString("inneractive.status").equals("on"))) {
            DCPAjillionAdnetwork adaptor = new DCPAjillionAdnetwork(logger, config, clientBootstrap, base, serverEvent);
            adaptor.setName("inneractive");
            return adaptor;
        }
        else if ((advertiserId.equals(config.getString("webmedia.advertiserId")))
                && (advertiserSet.isEmpty() || advertiserSet.contains("webmedia"))
