...
public class AuctionEngine implements AuctionEngineInterface {
  ...
  /***
   * RunRtbSecondPriceAuctionEngine returns the adnetwork selected after auctioning If no of rtb segments selected after
   * filtering is zero it returns the null If no of rtb segments selected after filtering is one it returns the rtb
   * adapter for the segment BidFloor is maximum of lowestEcpm and siteFloor If only 2 rtb are selected, highest bid
   * will win and would be charged the secondHighest price If only 1 rtb is selected, it will be selected for sending
   * response and will be charged the highest of secondHighest price or 90% of bidFloor
   * 
  ...
  public synchronized AdNetworkInterface runAuctionEngine() {
    ...
    if (filteredChannelSegmentList.isEmpty()) {
      ...
      if (DemandSourceType.RTBD == auctionResponse.getAdNetworkInterface().getDst()) {
        // For RTBD
        secondBidPrice =
            Math.min(casInternalRequestParameters.getAuctionBidFloor(), auctionResponse.getAdNetworkInterface()
                .getBidPriceInUsd());
      } else {
        // For IX,
  ...
  public void updateIXChannelSegment(final ChannelSegmentEntity dspChannelSegmentEntity) {
    auctionResponse =
        new ChannelSegment(dspChannelSegmentEntity, null, null, null, null, auctionResponse.getAdNetworkInterface(),
            -1L);
  }
  ...
  /**
   * Returns auctionFloor which is the maximum of siteFloor, highestEcpm, segmentFloor, countryFloor and networkSiteEcpm
   * 
   * @param siteFloor
   * @param highestEcpm
   * @param segmentFloor
