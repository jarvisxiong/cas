...
public class Filters {
	...
	public Filters(Map<String, HashMap<String, ChannelSegment>> matchedSegments,
	    ...
	    SASRequestParameters sasParams, RepositoryHelper repositoryHelper, DebugLogger logger) {
	...
	boolean isAdvertiserBurnLimitExceeded(ChannelSegment channelSegment) {
		boolean result = channelSegment.getChannelFeedbackEntity().getBalance() < channelSegment
		    .getChannelFeedbackEntity().getRevenue() * revenueWindow;
	...
	boolean isAdvertiserDailyRequestCapExceeded(ChannelSegment channelSegment) {
		boolean result = channelSegment.getChannelFeedbackEntity().getTodayRequests() > channelSegment
		    .getChannelEntity().getRequestCap();
		String advertiserId = channelSegment.getChannelSegmentEntity().getAdvertiserId();
		if (result) {
	...
	boolean isAdvertiserExcluded(ChannelSegment channelSegment) {
		...
		String advertiserId = channelSegment.getChannelSegmentEntity().getAdvertiserId();
		SiteMetaDataEntity siteMetaDataEntity = repositoryHelper.querySiteMetaDetaRepository(sasParams
		    .getSiteId());
	...
	boolean isAdGroupDailyImpressionCeilingExceeded(ChannelSegment channelSegment) {
		...
		String advertiserId = channelSegment.getChannelSegmentEntity().getAdvertiserId();
		if (result) {
			logger.debug("Impression limit exceeded by adgroup", channelSegment.getChannelSegmentEntity()
			...
			logger.debug("Impression limit filter passed by adgroup", channelSegment
			    .getChannelSegmentEntity().getAdgroupId(), "advertiser", advertiserId);
		}
		return result;
	}
	...
	void advertiserLevelFiltering() {
		...
		for (Map.Entry<String, HashMap<String, ChannelSegment>> advertiserEntry : matchedSegments
			...
			if (isAdvertiserBurnLimitExceeded(channelSegment)
			    ...
			    || isAdvertiserExcluded(channelSegment)) {
				continue;
			}
	...
	void adGroupLevelFiltering() {
		...
		for (Map.Entry<String, HashMap<String, ChannelSegment>> advertiserEntry : matchedSegments
			...
			for (Map.Entry<String, ChannelSegment> adGroupEntry : adGroups.entrySet()) {
				...
				if (isModelIdNotSupportedByAdGroup(channelSegment)) {
					continue;
				}
	...
	private boolean isModelIdNotSupportedByAdGroup(ChannelSegment channelSegment) {
		boolean result = false;
		String advertiserId = channelSegment.getChannelSegmentEntity().getAdvertiserId();
		result = null != channelSegment.getChannelSegmentEntity().getManufModelTargetingList()
	...
	boolean isAnySegmentPropertyViolated(ChannelSegmentEntity channelSegmentEntity) {
		if (channelSegmentEntity.isUdIdRequired()
			...
			InspectorStats.incrementStatCount(
			    advertiserIdtoNameMapping.get(channelSegmentEntity.getAdvertiserId()),
	...
	List<ChannelSegment> selectTopAdgroupsForRequest(List<ChannelSegment> rows) {
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			if (totalSegments < totalSegmentNo) {
				...
				totalSegments++;
			} else if (advertiserIdtoNameMapping.containsKey(channelSegment.getChannelSegmentEntity()
			    .getAdvertiserId())) {
				InspectorStats.incrementStatCount(advertiserIdtoNameMapping.get(channelSegment
		...
		for (int i = 0; i < shortlistedRow.size(); i++) {
			if (logger.isDebugEnabled()) {
				logger.debug("Segment with advertiserid "
				    + shortlistedRow.get(i).getChannelSegmentEntity().getAdvertiserId() + " adroupid "
				    + shortlistedRow.get(i).getChannelSegmentEntity().getAdgroupId() + " Pecpm "
	...
	double calculatePrioritisedECPM(ChannelSegment channelSegment) {
		...
		int priority = channelSegment.getChannelEntity().getPriority() < 5 ? 5 - channelSegment
		    .getChannelEntity().getPriority() : 1;
			...
			for (Map.Entry<String, HashMap<String, ChannelSegment>> advertiserEntry : matchedSegments
				...
				for (Map.Entry<String, ChannelSegment> adGroupEntry : adGroups.entrySet()) {
					...
					logger.debug("Advertiser is", channelSegment.getChannelSegmentEntity().getAdvertiserId(),
					    "and AdGp is", channelSegment.getChannelSegmentEntity().getAdgroupId(), "ecpm is",
	...
	public List<ChannelSegment> rankAdapters(List<ChannelSegment> segment) {
		...
		while (segment.size() > 1) {
			...
			for (int index = 0; index < segment.size(); index++) {
				if (randomNumber > segment.get(index).getPrioritisedECPM()) {
					...
					logger.debug("rank", rank++, "adapter has channel id", segment.get(index)
					    .getAdNetworkInterface().getId());
		...
		logger.debug("rank", rank, "adapter has channel id", segment.get(0).getAdNetworkInterface()
		    .getId());
		...
		for (Map.Entry<String, HashMap<String, ChannelSegment>> advertiserEntry : matchedSegments
			...
			for (Map.Entry<String, ChannelSegment> adGroupEntry : adGroups.entrySet()) {
				...
				logger.debug("ChannelSegment Added to list for advertiserid :", channelSegment
				    .getChannelSegmentEntity().getAdvertiserId(), "and adgroupid", channelSegment
	...
	public List<ChannelSegment> ensureGuaranteedDelivery(List<ChannelSegment> rankList) {
		...
		for (int rank = 1; rank < rankList.size(); rank++) {
			...
			if (!adapterConfiguration.getString(
			    rankedSegment.getAdNetworkInterface().getName() + ".gauranteedDelivery", "false").equals(
			    "true")) {
				newRankList.add(rankedSegment);
			} else {
				logger.debug("Dropping partner", rankedSegment.getAdNetworkInterface().getName(), "rank",
				    rank, "due to guarnteed delivery");
				InspectorStats.incrementStatCount(advertiserIdtoNameMapping.get(rankedSegment
				    .getChannelSegmentEntity().getAdvertiserId()),
