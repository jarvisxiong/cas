...
public class Filters {
    ...
    public List<ChannelSegment> applyFilters() {
        ...
        if (matchedSegments.isEmpty()) {
            LOG.debug(traceMarker, "No adGroups left for filtering , so returning empty list");
    ...
    private void sumUpSiteImpressions() {
        ...
        LOG.debug(traceMarker, "Site impressions:{}", this.siteImpressions);
    }
    ...
    boolean isAdvertiserBurnLimitExceeded(final ChannelSegment channelSegment) {
        ...
        if (result) {
            LOG.debug(traceMarker, "Burn limit exceeded by advertiser {}", advertiserId);
            channelSegment.incrementInspectorStats(InspectorStrings.droppedInburnFilter);
        }
        else {
            LOG.debug(traceMarker, "Burn limit filter passed by advertiser {}", advertiserId);
        }
        return result;
    }
    ...
    boolean isAdvertiserDroppedInRtbBalanceFilter(final ChannelSegment channelSegment) {
        ...
        if (result) {
            LOG.debug(traceMarker, "Balance is less than {} dollars for advertiser {}", rtbBalanceFilterAmount,
                advertiserId);
            channelSegment.incrementInspectorStats(InspectorStrings.droppedInRtbBalanceFilter);
        }
        else {
            LOG.debug(traceMarker, "RTB balance filter passed by advertiser {}", advertiserId);
        }
        return result;
    }
    ...
    boolean isAdvertiserDailyImpressionCeilingExceeded(final ChannelSegment channelSegment) {
        ...
        String advertiserId = channelSegment.getChannelSegmentEntity().getAdvertiserId();
        if (result) {
            LOG.debug(traceMarker, "Impression limit exceeded by advertiser{}", advertiserId);
            channelSegment.incrementInspectorStats(InspectorStrings.droppedInImpressionFilter);
        }
        else {
            LOG.debug(traceMarker, "Impression limit filter passed by advertiser {}", advertiserId);
        }
        return result;
    }
    ...
    boolean isAdvertiserDailyRequestCapExceeded(final ChannelSegment channelSegment) {
        ...
        String advertiserId = channelSegment.getChannelSegmentEntity().getAdvertiserId();
        if (result) {
            LOG.debug(traceMarker, "Request Cap exceeded by advertiser {}", advertiserId);
            channelSegment.incrementInspectorStats(InspectorStrings.droppedInRequestCapFilter);
        }
        else {
            LOG.debug(traceMarker, "Request Cap filter passed by advertiser {}", advertiserId);
        }
        return result;
    }
    ...
    boolean isAdvertiserExcluded(final ChannelSegment channelSegment) {
        ...
        if (result) {
            LOG.debug(traceMarker, "Dropping in Advertiser Inclusion Filter, advertiser {}", advertiserId);
            channelSegment.incrementInspectorStats(InspectorStrings.droppedinAdvertiserInclusionFilter);
        }
        else {
            LOG.debug(traceMarker, "Advertiser Inclusion filter passed by advertiser {}", advertiserId);
        }
        return result;
    }
    ...
    boolean isSiteExcludedByAdvertiser(final ChannelSegment channelSegment) {
        ...
        if (result) {
            LOG.debug(traceMarker, "Dropping as site not present in inclusion list of advertiser {}", advertiserId);
            channelSegment.incrementInspectorStats(InspectorStrings.droppedinSiteInclusionFilter);
        }
        else {
            LOG.debug(traceMarker, "Site inclusion exclusion passed by advertiser {}", advertiserId);
        }
        return result;
    }
    ...
    boolean isSiteExcludedByAdGroup(final ChannelSegment channelSegment) {
        ...
        if (result) {
            LOG.debug(traceMarker, "Dropping as site not present in inclusion list of adroup {}", adGroupId);
            channelSegment.incrementInspectorStats(InspectorStrings.droppedinSiteInclusionFilter);
        }
        else {
            LOG.debug(traceMarker, "Site inclusion exclusion passed by adgroup {}", adGroupId);
        }
        return result;
    }
    ...
    boolean isAdGroupDailyImpressionCeilingExceeded(final ChannelSegment channelSegment) {
        ...
        String advertiserId = channelSegment.getChannelSegmentEntity().getAdvertiserId();
        if (result) {
            LOG.debug(traceMarker, "Impression limit exceeded by adgroup {} advertiser {}", channelSegment
                    .getChannelSegmentEntity()
                        .getAdgroupId(), advertiserId);
            channelSegment.incrementInspectorStats(InspectorStrings.droppedInImpressionFilter);
        }
        else {
            LOG.debug(traceMarker, "Impression limit filter passed by adgroup {} advertiser {}", channelSegment
                    .getChannelSegmentEntity()
                        .getAdgroupId(), advertiserId);
        }
        return result;
    ...
    void advertiserLevelFiltering() {
        ...
        LOG.debug(traceMarker, "Inside advertiserLevelFiltering");
    ...
    void adGroupLevelFiltering() {
        ...
        LOG.debug(traceMarker, "Inside adGroupLevelFiltering");
        Map<String, HashMap<String, ChannelSegment>> rows = new HashMap<String, HashMap<String, ChannelSegment>>();
        supplyClass = getSupplyClass(sasParams);
        LOG.debug(traceMarker, "Supply class is {}", supplyClass);
        for (Map.Entry<String, HashMap<String, ChannelSegment>> advertiserEntry : matchedSegments.entrySet()) {
            String advertiserId = advertiserEntry.getKey();
            ...
            for (Map.Entry<String, ChannelSegment> adGroupEntry : adGroups.entrySet()) {
                ...
                if (channelSegment.getChannelSegmentFeedbackEntity().getECPM() < sasParams.getSiteFloor()) {
                    LOG.debug(traceMarker, "sitefloor filter failed by adgroup {}", channelSegment
                            .getChannelSegmentFeedbackEntity()
                                .getId());
                    continue;
                }
                else {
                    LOG.debug(traceMarker, "sitefloor filter passed by adgroup {}", channelSegment
                            .getChannelSegmentFeedbackEntity()
                                .getId());
                }
                ...
                if (null != channelSegment.getChannelSegmentEntity().getManufModelTargetingList()
                    ...
                    LOG.debug(traceMarker, "{} dropped in model id filter", channelSegment
                            .getChannelSegmentEntity()
                                .getId());
            ...
            LOG.debug(traceMarker, "PartnersegmentNo for advertiser {} is {}", advertiserId, partnerSegmentNo);
    ...
    boolean isAdvertiserFailedInAccountSegmentFilter(final ChannelSegment channelSegment) {
        ...
        if (sasParams.getDst() == 6 && null != sasParams.getAccountSegment()
            ...
            LOG.debug(traceMarker, "Account segment filter failed by advertiser {}", advertiserId);
            return true;
        }
        LOG.debug(traceMarker, "Account segment filter passed by advertiser {}", advertiserId);
        return false;
    }
    ...
    boolean isDroppedInDstFilter(final String advertiserId, final ChannelSegment channelSegment) {
        ...
        if (sasParams.getDst() == 6 && channelSegment.getChannelSegmentEntity().getDst() != sasParams.getDst()) {
            LOG.debug(traceMarker, "dropped in dst filter for advertiser {}", advertiserId);
            if (getAdvertiserIdToNameMapping().containsKey(advertiserId)) {
                InspectorStats.incrementStatCount(getAdvertiserIdToNameMapping().get(advertiserId),
        ...
        LOG.debug(traceMarker, "dst filter passed for advertiser {}", advertiserId);
        return false;
    }
    ...
    byte getSupplyClass(final SASRequestParameters sasParams) {
        ...
        if (siteEcpmEntity == null) {
            ...
            LOG.debug(traceMarker, "Number of supply classes {}", supplyClassFloors.length);
            LOG.debug(traceMarker, "SupplyClassFloors {} ", (Object[]) supplyClassFloors);
    ...
    boolean isDemandAcceptedBySupply(final ChannelSegment channelSegment) {
        ...
        LOG.debug(traceMarker, "Demand class is {} for adgroup {}", demandClass, channelSegment
                .getChannelSegmentEntity()
                    .getAdgroupId());
        ...
        String advertiserId = channelSegment.getChannelSegmentEntity().getAdvertiserId();
        if (!result) {
            LOG.debug(traceMarker, "Supply does not accepts demand");
    ...
    boolean isTODTargetingFailed(final ChannelSegment channelSegment) {
        ...
        if (null == channelSegment.getChannelSegmentEntity().getTod()) {
            LOG.debug(traceMarker, "{} has all ToD and DoW targeting. Passing the ToD check ", channelSegment
                    .getChannelSegmentEntity()
                        .getAdgroupId());
            return false;
        }
        ...
        LOG.debug(traceMarker, "dayOfWeek is : {} hourOfDay is : {} todt calculated is : {}", dayOfWeek, hourOfDay,
            todt);
        if (todt == 0) {
            LOG.debug(traceMarker, "Hour of day targeting failed. Returning true");
            channelSegment.incrementInspectorStats(InspectorStrings.droppedInTODFilter);
            return true;
        }
        LOG.debug(traceMarker, "Hour of day targeting passed. Returning false");
        return false;
    ...
    boolean isChannelSegmentFilteredOutByPricingEngine(final String advertiserId, final Double dcpFloor,
        ...
        if (null != dcpFloor) {
            ...
            if (null != eCPMBoostExpiryDate && eCPMBoostExpiryDate.compareTo(today) > 0) {
                LOG.debug(traceMarker, "EcpmBoost is applied for {}", channelSegment
                        .getChannelSegmentEntity()
                            .getAdgroupId());
            ...
            if (ServletHandler.random.nextInt(100) <= percentage) {
                ...
                LOG.debug(traceMarker, "dcp floor filter failed by adgroup {}", channelSegment
                        .getChannelSegmentFeedbackEntity()
                            .getId());
    ...
    List<ChannelSegment> selectTopAdGroupsForRequest(final List<ChannelSegment> rows) {
        ...
        LOG.debug(traceMarker, "Inside selectTopAdGroupsForRequest Filter");
        ...
        LOG.debug(traceMarker, "Number of  ShortListed Segments are : {}", Integer
                .valueOf(shortlistedRow.size())
                    .toString());
        for (ChannelSegment aShortlistedRow : shortlistedRow) {
            LOG.debug(traceMarker, "Segment with advertiserId {} adGroupId {} Pecpm {}", aShortlistedRow
                    .getChannelSegmentEntity()
    ...
    void printSegments(final Map<String, HashMap<String, ChannelSegment>> matchedSegments) {
        ...
        if (LOG.isDebugEnabled()) {
            ...
            for (Map.Entry<String, HashMap<String, ChannelSegment>> advertiserEntry : matchedSegments.entrySet()) {
                ...
                for (Map.Entry<String, ChannelSegment> adGroupEntry : adGroups.entrySet()) {
                    ChannelSegment channelSegment = adGroupEntry.getValue();
                    LOG.debug(traceMarker, "Advertiser is {} and AdGp is {}", channelSegment
                            .getChannelSegmentEntity()
    ...
    public List<ChannelSegment> rankAdapters(final List<ChannelSegment> segment) {
        ...
        while (segment.size() > 1) {
            ...
            for (int index = 0; index < segment.size(); index++) {
                if (randomNumber > segment.get(index).getPrioritisedECPM()) {
                    ...
                    LOG.debug(traceMarker, "rank {} adapter has channel id {}", rank++, segment
                            .get(index)
        ...
        LOG
                .debug(traceMarker, "rank {} adapter has channel id{}", rank, segment
                        .get(0)
                            .getAdNetworkInterface()
                            .getId());
        rankedList.add(segment.get(0));
        LOG.info(traceMarker, "Ranked candidate adapters randomly");
        return rankedList;
    }
    ...
    List<ChannelSegment> convertToSegmentsList(final Map<String, HashMap<String, ChannelSegment>> matchedSegments) {
        ...
        for (Map.Entry<String, HashMap<String, ChannelSegment>> advertiserEntry : matchedSegments.entrySet()) {
            ...
            for (Map.Entry<String, ChannelSegment> adGroupEntry : adGroups.entrySet()) {
                ...
                LOG.debug(traceMarker, "ChannelSegment Added to list for advertiserid : {} and adgroupid {}",
                    channelSegment.getChannelSegmentEntity().getAdvertiserId(), channelSegment
                            .getChannelSegmentEntity()
                                .getAdgroupId());
            }
        }
    ...
    public List<ChannelSegment> ensureGuaranteedDelivery(final List<ChannelSegment> rankList) {
        ...
        LOG.debug(traceMarker, "Inside guaranteed delivery filter");
        ...
        for (int rank = 1; rank < rankList.size(); rank++) {
            ...
            if (!adapterConfiguration.getString(
                ...
                LOG.debug(traceMarker, "Dropping partner {} rank {} due to guarnteed delivery", rankedSegment
                        .getAdNetworkInterface()
                            .getName(), rank);
                rankedSegment.incrementInspectorStats(InspectorStrings.droppedInGuaranteedDelivery);
            }
        }
        LOG.debug(traceMarker, "New ranklist size :" + newRankList.size());
        return newRankList;
    }
    ...
    public List<ChannelSegment> ensureGuaranteedDeliveryInCaseOfRTB(final List<ChannelSegment> rtbSegments,
        ...
        LOG.debug(traceMarker, "Inside guaranteed delivery RTB filter");
        if (!rankList.isEmpty()
                && adapterConfiguration.getString(
                    rankList.get(0).getAdNetworkInterface().getName() + ".gauranteedDelivery", "false").equals("true")) {
            rtbSegments.clear();
            LOG.debug(traceMarker, "Dropped all RTB segments due to guaranteed delivery RTB filter");
        }
        else {
            LOG.debug(traceMarker, "All RTB segments passed guaranteed delivery RTB filter");
        }
        return rtbSegments;
    }
