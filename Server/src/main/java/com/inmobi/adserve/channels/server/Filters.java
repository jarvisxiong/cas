...
public class Filters {
  ...
  public static HashMap<String, HashMap<String, ChannelSegmentEntity>> impressionBurnFilter(
    ...
    for (String advertiserId : matchedSegments.keySet()) {
      ...
      channelId = ((ChannelSegmentEntity[]) matchedSegments.get(advertiserId).values()
          .toArray(new ChannelSegmentEntity[0]))[0].getChannelId();
      ...
      if(todayImpressions == -1 || impressionCeil == -1)
        logger
            .debug("Repo Exception/No entry in ChannelFeedbackRepository/ChannelRepository for advertiserID "
                + advertiserId);
      else {
        if(todayImpressions > impressionCeil) {
          ...
          if(advertiserIdtoNameMapping.containsKey(advertiserId))
            inspectorStat.incrementStatCount(advertiserIdtoNameMapping.get(advertiserId),
                "filter_stat", InspectorStrings.droppedInImpressionFilter);
          continue;
        }
      }
      ...
      if(balance == -1)
        logger.debug("Repo Exception/No entry in ChannelFeedbackRepository for advertiserID "
            + advertiserId);
      else {
        if(balance < revenue * revenueWindow) {
          ...
          if(advertiserIdtoNameMapping.containsKey(advertiserId))
            inspectorStat.incrementStatCount(advertiserIdtoNameMapping.get(advertiserId),
                "filter_stat", InspectorStrings.droppedInburnFilter);
          continue;
        }
        logger.debug("Burn limit filter passed by advertiser " + advertiserId + " "
            + repositoryHelper.queryChannelFeedbackRepository(advertiserId).getRevenue()
            * revenueWindow);
      }
  ...
  public static HashMap<String, HashMap<String, ChannelSegmentEntity>> partnerSegmentCountFilter(
      HashMap<String, HashMap<String, ChannelSegmentEntity>> matchedSegments, Double siteFloor,
      DebugLogger logger, Configuration serverConfiguration, Configuration adapterConfiguration) {
    ...
    for (String advertiserId : matchedSegments.keySet()) {
      ...
      for (String adgroupId : matchedSegments.get(advertiserId).keySet()) {
        ChannelSegmentEntity channelSegmentEntity = matchedSegments.get(advertiserId)
            .get(adgroupId);
        ...
        if(null == channelSegmentFeedbackEntity) {
          ...
          channelSegmentFeedbackEntity = new ChannelSegmentFeedbackEntity(
              channelSegmentEntity.getId(), channelSegmentEntity.getAdgroupId(),
        ...
        if(channelSegmentFeedbackEntity.geteCPM() >= siteFloor) {
          logger
              .debug("sitefloor filter passed by adgroup " + channelSegmentFeedbackEntity.getId());
          hashMapList.add(channelSegmentFeedbackEntity);
        } else
          logger
              .debug("sitefloor filter failed by adgroup " + channelSegmentFeedbackEntity.getId());
      }
  ...
  public static ChannelSegmentEntity[] segmentsPerRequestFilter(
    ...
    for (ChannelSegmentEntity row : rows) {
      ...
      channelSegmentFeedbackEntity = repositoryHelper.queryChannelSegmentFeedbackRepository(row
          .getAdgroupId());
      if(null == channelSegmentFeedbackEntity) {
        logger.debug("Error in retreiving from repo for adgprid " + row.getAdgroupId()
            + " and advertiserid " + row.getId() + " so setting ecpm to default");
        channelSegmentFeedbackEntity = new ChannelSegmentFeedbackEntity(row.getId(),
            row.getAdgroupId(), serverConfiguration.getDouble("default.ecpm"),
            serverConfiguration.getDouble("default.fillratio"));
      }
      ...
      channelSegmentFeedbackEntity.setPrioritisedECPM(Math.pow(
          (channelSegmentFeedbackEntity.geteCPM() + eCPMShift), feedbackPower)
    ...
    for (int i = 0; i < hashMapList.size(); i++) {
      ...
      if(totalSegments < totalSegmentNo) {
        ...
        inspectorStat.incrementStatCount(advertiserIdtoNameMapping.get(advertiserId),
            "filter_stat", InspectorStrings.droppedInSegmentPerRequestFilter);
    }
    logger.debug("Number of  ShortListed Segments are : " + shortlistedRow.size());
    for (int i = 0; i < shortlistedRow.size(); i++) {
      Double eCPM;
      if(null != repositoryHelper.queryChannelSegmentFeedbackRepository(shortlistedRow.get(i)
          .getAdgroupId()))
        eCPM = repositoryHelper.queryChannelSegmentFeedbackRepository(
            shortlistedRow.get(i).getAdgroupId()).getPrioritisedECPM();
      else {
  ...
  public static void printSegments(
    ...
    for (String adkey : matchedSegments.keySet()) {
      for (String gpkey : matchedSegments.get(adkey).keySet()) {
        try {
          logger.debug("Advertiser is " + matchedSegments.get(adkey).get(gpkey).getId()
              + " and AdGp is " + matchedSegments.get(adkey).get(gpkey).getAdgroupId()
              + " \tecpm is "
              + repositoryHelper.queryChannelSegmentFeedbackRepository(gpkey).geteCPM());
        } catch (NullPointerException e) {
          logger.debug("Repo Exception/No entry in ChannelSegmentFeedbackRepository for adgrpId : "
              + gpkey);
          continue;
        }
      }
  ...
  public static ArrayList<ChannelSegment> rankAdapters(List<ChannelSegment> segment,
      DebugLogger logger, Configuration serverConfiguration) {
    ...
    while (segment.size() > 1) {
      ...
      for (int index = 0; index < segment.size(); index++) {
        ...
        segment.get(index).channelSegmentFeedbackEntity
            .setPrioritisedECPM((Math.pow(
                (segment.get(index).channelSegmentFeedbackEntity.geteCPM() + eCPMShift),
                feedbackPower) * (segment.get(index).channelEntity.getPriority() < 5 ? (5 - segment
                .get(index).channelEntity.getPriority()) : 1))
        ...
        if(randomNumber >= segment.get(index).lowerPriorityRange
            && randomNumber <= segment.get(index).higherPriorityRange) {
          {
            logger.debug("rank " + rank++ + " adapter has channel id "
                + segment.get(index).adNetworkInterface.getId());
          }
    ...
    logger.debug("rank " + rank++ + " adapter has channel id "
        + segment.get(0).adNetworkInterface.getId());
