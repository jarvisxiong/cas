...
public abstract class AbstractAdvertiserLevelFilter implements AdvertiserLevelFilter {
  ...
  public final void filter(final List<AdvertiserMatchedSegmentDetail> matchedSegmentDetails,
    ...
    for (final Iterator<AdvertiserMatchedSegmentDetail> iterator = matchedSegmentDetails.iterator(); iterator.hasNext();) {
      ...
      if (result) {
        iterator.remove();
        LOG.debug(traceMarker, "Failed in filter {}  , advertiser {}", this.getClass().getSimpleName(), advertiserId);
