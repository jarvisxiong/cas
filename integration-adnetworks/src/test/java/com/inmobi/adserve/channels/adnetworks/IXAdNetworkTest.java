...
public class IXAdNetworkTest extends TestCase {
    ...
    public void prepareMockConfig() {
        ...
        expect(mockConfig.getStringArray("ix.blockedAdvertisers")).andReturn(new String[]{"king.com","supercell.net", "paps.com", "fhs.com", "china.supercell.com", "supercell.com"}).anyTimes();
        expect(mockConfig.getList("ix.globalBlind")).andReturn(new ArrayList<String>(Arrays.asList("1", "2"))).anyTimes();
        replay(mockConfig);
    }
    ...
    public void setUp() throws Exception {
        ...
        ixAdNetwork = new IXAdNetwork(mockConfig, null, base, serverChannel, urlBase, "ix", 200, repositoryHelper, true);

        Field asyncHttpClientProviderField = IXAdNetwork.class.getDeclaredField("asyncHttpClientProvider");
    ...
    public void testShouldNotConfigureAdapterIfSiteIdNotSentInAdditionalParams() {
        ...
        try {
            ChannelSegmentEntity entity = new ChannelSegmentEntity(AdNetworksTest.getChannelSegmentEntityBuilder(ixAdvId,
                    null, null, null, 0, null, null, true, true, externalSiteKey, null, null, null, new Long[]{0L}, true, null, null, 0,
                    null, false, false, false, false, false, false, false, false, false, false, new JSONObject("{\"3\":\"160212\"}"), new ArrayList<Integer>(),
    ...
    public void testShouldNotConfigureAdapterIfZoneIdNotSentInAdditionalParams() {
        ...
        try {
            ChannelSegmentEntity entity = new ChannelSegmentEntity(AdNetworksTest.getChannelSegmentEntityBuilder(ixAdvId,
                    null, null, null, 0, null, null, true, true, externalSiteKey, null, null, null, new Long[]{0L}, true, null, null, 0,
                    null, false, false, false, false, false, false, false, false, false, false, new JSONObject("{\"site\":\"38132\"}"), new ArrayList<Integer>(),
                    0.0d, null, null, 0, new Integer[]{0}));
    ...
    public void testShouldSetSiteOrAppNotBoth() {
        ...
        try {
            ...
            assertEquals(ixAdNetwork.getBidRequest().getSite().getBlocklists(),Lists.newArrayList("blk423","InMobiPERF"));
            ...
            //assertEquals(ixAdNetwork.getBidRequest().getSite().getId(), getBlindedSiteId(sasParams.getSiteIncId(), entity.getIncId(getCreativeType())));
            assertEquals(ixAdNetwork.getBidRequest().getSite().getName(),"Games");
            assertNull(ixAdNetwork.getBidRequest().getSite().getPage());
            assertEquals(ixAdNetwork.getBidRequest().getSite().getBlocklists(),Lists.newArrayList("blk423","InMobiFS"));
            assertEquals(ixAdNetwork.getBidRequest().getSite().getPublisher().getExt().getRp().getAccount_id(),11726);
...
/*
    ...
    public void testParseResponse() throws TException {
        ...
        ixAdNetwork.configureParameters(sasParams, casInternalRequestParameters, entity, clickUrl, beaconUrl);
        TSerializer serializer = new TSerializer(new TSimpleJSONProtocol.Factory());
        ixAdNetwork.parseResponse(serializer.toString(bidResponse), HttpResponseStatus.OK);
        assertEquals(responseAdm.toString(), ixAdNetwork.responseContent);
        ixAdNetwork.setEncryptedBid("0.23");
        ixAdNetwork.setSecondBidPrice(0.23);
        String afterMacros = ixAdNetwork.replaceIXMacros(responseAdm.toString());
        assertEquals(afterMacros, ixAdNetwork.responseContent);
        ixAdNetwork.parseResponse(str.toString(), HttpResponseStatus.NOT_FOUND);
        assertEquals("", ixAdNetwork.responseContent);
    }
