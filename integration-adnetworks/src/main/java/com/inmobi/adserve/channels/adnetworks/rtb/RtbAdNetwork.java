...
public class RtbAdNetwork extends BaseAdNetworkImpl {
	...
	private static List<String> mimes = Arrays.asList("image/jpeg",
			"image/gif", "image/png");
	private static List<Integer> fsBlockedAttributes = Arrays.asList(1, 2, 3,
			4, 5, 6, 7, 8, 9, 13, 15, 16);
	private static List<Integer> performanceBlockedAttributes = Arrays.asList(
	...
	@Getter
	static List<String> currenciesSupported = new ArrayList<String>(
			Arrays.asList("USD", "RMB"));
	@Inject
	...
	public RtbAdNetwork(final Configuration config,
			final Bootstrap clientBootstrap,
			final HttpRequestHandlerBase baseRequestHandler,
			final Channel serverChannel, final String urlBase,
			final String advertiserName, final int tmax,
	...
	protected boolean configureParameters() {
		...
		} else if (null != sasParams.getAdcode()
				&& "JS".equalsIgnoreCase(sasParams.getAdcode())) {
			displayManager = DISPLAY_MANAGER_INMOBI_JS;
		}
		Impression impression = createImpressionObject(banner, displayManager,
		...
		boolean flag = createBidRequestObject(impresssionlist, site, app, user,
				device);
		if (!flag) {
			return false;
		}
	...
	private boolean createBidRequestObject(
			final List<Impression> impresssionlist, final Site site,
			final App app, final User user, final Device device) {
		bidRequest = new BidRequest(casInternalRequestParameters.auctionId,
				impresssionlist);
		...
		if (casInternalRequestParameters != null) {
			LOG.debug("blockedCategories are {}",
					casInternalRequestParameters.blockedCategories);
			LOG.debug("blockedAdvertisers are {}",
					casInternalRequestParameters.blockedAdvertisers);
			bidRequest.setBcat(new ArrayList<String>());
			if (null != casInternalRequestParameters.blockedCategories) {
				bidRequest
						.setBcat(iabCategoriesInterface
								.getIABCategories(casInternalRequestParameters.blockedCategories));
			}
			// Setting blocked categories
			if (SITE_RATING_PERFORMANCE.equalsIgnoreCase(sasParams
					.getSiteType())) {
				bidRequest
						.getBcat()
						.addAll(iabCategoriesInterface
								.getIABCategories(IABCategoriesMap.PERFORMANCE_BLOCK_CATEGORIES));
			} else {
				bidRequest
						.getBcat()
						.addAll(iabCategoriesInterface
								.getIABCategories(IABCategoriesMap.FAMILY_SAFE_BLOCK_CATEGORIES));
			}

			if (null != casInternalRequestParameters.blockedAdvertisers) {
				bidRequest
						.setBadv(casInternalRequestParameters.blockedAdvertisers);
			}
		} else {
	...
	private boolean serializeBidRequest() {
		...
		try {
			...
			LOG.debug("Could not create json from bidrequest for partner {}",
					advertiserName);
			LOG.info("Configure parameters inside rtb returned false {}",
					advertiserName);
			return false;
		}
	...
	private Impression createImpressionObject(final Banner banner,
			final String displayManager, final String displayManagerVersion) {
		...
		if (null != sasParams.getRqAdType()
				&& "int".equalsIgnoreCase(sasParams.getRqAdType())) {
	...
	private Banner createBannerObject() {
		...
		if (!StringUtils.isEmpty(sasParams.getSdkVersion())
				&& sasParams.getSdkVersion().length() > 1) {
	...
	private Geo createGeoObject() {
		...
		if (null != sasParams.getCountryCode()) {
			geo.setCountry(iabCountriesInterface.getIabCountry(sasParams
					.getCountryCode()));
		}
	...
	private Site createSiteObject() {
		Site site = null;
		if (siteBlinded) {
			site = new Site(getBlindedSiteId(sasParams.getSiteIncId(),
					entity.getIncId()));
		} else {
			site = new Site(sasParams.getSiteId());
		}
		if (null != sasParams.getCategories()) {
			site.setCat(iabCategoriesInterface.getIABCategories(sasParams
					.getCategories()));
		}
	...
	private App createAppObject() {
		App app = null;
		if (siteBlinded) {
			app = new App(getBlindedSiteId(sasParams.getSiteIncId(),
					entity.getAdgroupIncId()));
		} else {
			app = new App(sasParams.getSiteId());
		}
		if (null != sasParams.getCategories()) {
			app.setCat(iabCategoriesInterface.getIABCategories(sasParams
					.getCategories()));
		}
	...
	private Device createDeviceObject(final Geo geo) {
		...
		if (null != casInternalRequestParameters.uidADT) {
			try {
				device.setDnt(Integer
						.parseInt(casInternalRequestParameters.uidADT) == 0 ? 1
						: 0);
			} catch (NumberFormatException e) {
		...
		if (!StringUtils.isEmpty(casInternalRequestParameters.uidIFA)) {
			...
			deviceExtensions
					.put("idfasha1",
							getHashedValue(casInternalRequestParameters.uidIFA,
									"SHA-1"));
			deviceExtensions.put("idfamd5",
					getHashedValue(casInternalRequestParameters.uidIFA, "MD5"));
	...
	public void impressionCallback() {
		...
		content.append("{\"bidid\"=").append(bidResponse.bidid)
				.append(",\"seat\"=")
		...
		Request ningRequest = new RequestBuilder()
				.setUrl(uriCallBack.toASCIIString())
				.setHeader(HttpHeaders.Names.CONTENT_TYPE, "application/json")
				.setHeader(HttpHeaders.Names.CONTENT_LENGTH,
						String.valueOf(body.length)).setBody(body)
				.setHeader(HttpHeaders.Names.HOST, uriCallBack.getHost())
				.build();

		boolean callbackResult = impressionCallbackHelper.writeResponse(
				uriCallBack, ningRequest, getAsyncHttpClient());
		if (callbackResult) {
	...
	private void setCallbackContent() {
		...
		content.append(",\"bid\"=")
				.append(bidResponse.seatbid.get(0).bid.get(0).id)
				.append(",\"price\"=").append(secondBidPriceInUsd)
				.append(",\"adid\"=")
				.append(bidResponse.seatbid.get(0).bid.get(0).adid).append("}");
	}
	...
	public String replaceRTBMacros(String url) {
		url = url.replaceAll(
				"(?i)" + Pattern.quote(RTBCallbackMacros.AUCTION_ID),
				bidResponse.id);
		url = url.replaceAll(
				"(?i)" + Pattern.quote(RTBCallbackMacros.AUCTION_CURRENCY),
				bidderCurrency);
		if (6 != sasParams.getDst()) {
			url = url
					.replaceAll(
							"(?i)"
									+ Pattern
											.quote(RTBCallbackMacros.AUCTION_PRICE_ENCRYPTED),
							encryptedBid);
			url = url.replaceAll(
					"(?i)" + Pattern.quote(RTBCallbackMacros.AUCTION_PRICE),
					Double.toString(secondBidPriceInLocal));
		}
		if (null != bidResponse.getSeatbid().get(0).getBid().get(0).getAdid()) {
			url = url.replaceAll(
					"(?i)" + Pattern.quote(RTBCallbackMacros.AUCTION_AD_ID),
					bidResponse.getSeatbid().get(0).getBid().get(0).getAdid());
		}
		if (null != bidResponse.bidid) {
			url = url.replaceAll(
					"(?i)" + Pattern.quote(RTBCallbackMacros.AUCTION_BID_ID),
					bidResponse.bidid);
		}
		if (null != bidResponse.getSeatbid().get(0).getSeat()) {
			url = url.replaceAll(
					"(?i)" + Pattern.quote(RTBCallbackMacros.AUCTION_SEAT_ID),
		...
		url = url.replaceAll(
				"(?i)" + Pattern.quote(RTBCallbackMacros.AUCTION_IMP_ID),
				bidRequest.getImp().get(0).getId());
	...
	protected Request getNingRequest() throws Exception {
		...
		return new RequestBuilder(httpRequestMethod).setURI(uri)
				.setHeader(HttpHeaders.Names.CONTENT_TYPE, "application/json")
				.setBody(body).setHeader(X_OPENRTB_VERSION, rtbVer)
	...
	public void parseResponse(final String response,
		...
		if (status.code() != 200 || StringUtils.isBlank(response)) {
			...
			if ("wap".equalsIgnoreCase(sasParams.getSource())) {
				velocityContext.put(
						VelocityTemplateFieldConstants.PartnerHtmlCode, image);
			} else {
				velocityContext.put(
						VelocityTemplateFieldConstants.PartnerHtmlCode, mraid
								+ image);
				if (StringUtils.isNotBlank(sasParams.getImaiBaseUrl())) {
					velocityContext.put(
							VelocityTemplateFieldConstants.IMAIBaseUrl,
							sasParams.getImaiBaseUrl());
				}
			}
			// Checking whether to send win notification
			LOG.debug("isWinRequired is {} and winfromconfig is {}",
					wnRequired, callbackUrl);
			if (wnRequired) {
				// setCallbackContent();
			...
			velocityContext.put(VelocityTemplateFieldConstants.IMBeaconUrl,
					this.beaconUrl);
			try {
				responseContent = Formatter
						.getResponseFromTemplate(TemplateType.RTB_HTML,
								velocityContext, sasParams, null);
			} catch (Exception e) {
				adStatus = "NO_AD";
				LOG.info(
						"Some exception is caught while filling the velocity template for partner{} {}",
						advertiserName, e);
			}
		}
	...
	public boolean deserializeResponse(final String response) {
		...
		try {
			...
			if (null == bidResponse || null == bidResponse.getSeatbid()
					|| bidResponse.getSeatbidSize() == 0) {
			...
			setBidPriceInLocal(bidResponse.getSeatbid().get(0).getBid().get(0)
					.getPrice());
			setBidPriceInUsd(calculatePriceInUSD(getBidPriceInLocal(),
					bidderCurrency));
			responseSeatId = bidResponse.getSeatbid().get(0).getSeat();
			responseImpressionId = bidResponse.getSeatbid().get(0).getBid()
					.get(0).getImpid();
			responseAuctionId = bidResponse.getId();
			return true;
		} catch (NullPointerException e) {
			LOG.info("Could not parse the rtb response from partner: {}",
					this.getName());
			return false;
		}
	}
	...
	private double calculatePriceInUSD(final double price, String currencyCode) {
		...
		if (USD.equalsIgnoreCase(currencyCode)) {
			...
			if (null != currencyConversionEntity
					&& null != currencyConversionEntity.getConversionRate()
	...
	public void setSecondBidPrice(final Double price) {
		...
		LOG.debug("responseContent after replaceMacros is {}",
				getResponseAd().response);
	}
