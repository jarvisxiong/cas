...
public class DCPWebmoblinkAdNetwork extends AbstractDCPAdNetworkImpl {
	private static final Logger LOG = LoggerFactory
			.getLogger(DCPWebmoblinkAdNetwork.class);
	...
	public DCPWebmoblinkAdNetwork(final Configuration config,
	...
	public URI getRequestUri() throws Exception {
		...
		appendQueryParam(url, USERAGENT,
		...
		if (sasParams.getCountryCode() != null) {
			appendQueryParam(url, COUNTRY_CODE, sasParams.getCountryCode(),
					false);
		}
		...
		if (isApp) {
			if (sasParams.getOsId() == HandSetOS.Android.getValue()) {
				...
				if (StringUtils.isNotBlank(casInternalRequestParameters
						.getUidMd5())) {
					...
					appendQueryParam(url, DEVICE_ID,
							casInternalRequestParameters.getUidO1(), false);
					appendQueryParam(url, DID_TYPE, 4, false);
				} else if (!StringUtils.isBlank(casInternalRequestParameters
						.getUid())) {
					appendQueryParam(url, DEVICE_ID,
				...
				if (StringUtils.isNotBlank(casInternalRequestParameters
				...
				} else if (StringUtils.isNotBlank(casInternalRequestParameters
						.getUidIDUS1())) {
					appendQueryParam(url, DEVICE_ID,
					...
					appendQueryParam(url, DEVICE_ID,
							casInternalRequestParameters.getUidSO1(), false);
					appendQueryParam(url, DID_TYPE, 3, false);
	...
	public void parseResponse(final String response,
		...
		if (null == response || status.code() != 200
			...
			try {
				ADResponse adResponse = jaxbHelper.unmarshal(response,
						ADResponse.class);
				...
				if (StringUtils.isNotBlank(adResponse.getAdText())) {
					context.put(VelocityTemplateFieldConstants.AD_TEXT,
							adResponse.getAdText());
				}
				context.put(VelocityTemplateFieldConstants.IM_CLICK_URL,
						clickUrl);
				context.put(VelocityTemplateFieldConstants.PARTNER_BEACON_URL,
						adResponse.getFirepixel());
				...
				adStatus = "NO_AD";
				LOG.info("Error parsing response from Webmoblink");
				LOG.info("Response from Webmoblink {}", response);
			}
		}
	}
