...
public class DCPRubiconAdnetwork extends AbstractDCPAdNetworkImpl {
  ...
  public URI getRequestUri() throws Exception {
    ...
    if (siteSpecificFloorDefault == null) {
      ...
      final Double siteSpecificFloorPerCountry =
          siteFloorMap.get(String.format(SITE_FLOOR_KEF_FORMAT, sasParams.getSiteIncId(), sasParams.getCountryId()));
      final double floor = siteSpecificFloorPerCountry == null ? siteSpecificFloorDefault : siteSpecificFloorPerCountry;
  ...
  public Request getNingRequest() throws Exception {
    ...
    return new RequestBuilder().setUrl(uri.toString())
        ...
        .setHeader("X-Forwarded-For", sasParams.getRemoteHostIp()).setHeader("Authorization", "Basic " + authEncoded)
    ...
    if (null == response || status.code() != 200 || response.trim().isEmpty()) {
      ...
      try {
        ...
        if ("ok".equalsIgnoreCase(adResponse.getString("status"))) {
          ...
          context.put(VelocityTemplateFieldConstants.PARTNER_HTML_CODE, String.format(RESPONSE_TEMPLATE, htmlContent));
