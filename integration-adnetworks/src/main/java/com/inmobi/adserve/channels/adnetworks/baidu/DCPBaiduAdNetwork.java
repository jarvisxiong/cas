...
public class DCPBaiduAdNetwork extends AbstractDCPAdNetworkImpl {
    ...
    private String getRequestParams() {
        ...
        final String category[] = getCategories(',', false, false).split(",");
        if (category.length > 0) {
            app.setCategory(category[0]);
        }

        final AdSlots adSlots = new AdSlots();
        final Size adSize = new Size();
        adSize.setHeight(height);
        adSize.setWidth(width);
        adSlots.setSize(adSize);
        adSlots.setId(adSlotId);

        final BaiduRequest request = new BaiduRequest();
        ...
        if (StringUtils.isNotEmpty(sasParams.getOsMajorVersion())) {
            final Version osVersionVal = new Version();
            final String[] osList = sasParams.getOsMajorVersion().split(".");
    ...
    public void parseResponse(final String response, final HttpResponseStatus status) {
        ...
        try {
            ...
            final BaiduResponse adResponse = gson.fromJson(response, BaiduResponse.class);
            if (adResponse.getAds() != null && StringUtils.isNotEmpty(adResponse.getAds().getHtml_snippet())) {
                context.put(VelocityTemplateFieldConstants.PARTNER_HTML_CODE, adResponse.getAds().getHtml_snippet());
            } else {
                adStatus = NO_AD;
                statusCode = 500;
                return;
