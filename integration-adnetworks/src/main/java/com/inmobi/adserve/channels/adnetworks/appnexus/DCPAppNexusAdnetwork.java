  ...
  public URI getRequestUri() throws Exception {
    try {
      ...
      if (StringUtils.isNotBlank(latitude) && StringUtils.isNotBlank(longitude)) {
        appendQueryParam(url, LOCATION, getURLEncode(String.format(LAT_LONG_FORMAT, latitude, longitude), format),
            false);
      }
      ...
      if (sasParams.getOsId() == HandSetOS.Android.getValue()) {
        if (StringUtils.isNotBlank(casInternalRequestParameters.getUidMd5())) {
          appendQueryParam(url, ANDROID_ID_MD5, getURLEncode(casInternalRequestParameters.getUidMd5(), format), false);
        ...
        if (StringUtils.isNotBlank(casInternalRequestParameters.getUidO1())) {
          appendQueryParam(url, ANDROID_ID_SHA1, getURLEncode(casInternalRequestParameters.getUidO1(), format), false);
        }
      ...
      if (sasParams.getOsId() == HandSetOS.iOS.getValue()) {
        ...
        if (StringUtils.isNotBlank(casInternalRequestParameters.getUidIDUS1())) {
          appendQueryParam(url, OPENUDID_SHA1, getURLEncode(casInternalRequestParameters.getUidIDUS1(), format), false);
        }
      }
