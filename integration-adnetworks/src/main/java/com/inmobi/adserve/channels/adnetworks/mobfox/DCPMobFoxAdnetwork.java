...
public class DCPMobFoxAdnetwork extends AbstractDCPAdNetworkImpl {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(DCPMobFoxAdnetwork.class);
	 ...
	 * @param serverEvent
	 */
	public DCPMobFoxAdnetwork(final Configuration config,
	...
	public URI getRequestUri() throws Exception {
		...
		if (StringUtils.isNotBlank(casInternalRequestParameters.uidSO1)) {
			appendQueryParam(url, SHA1UDID,
					casInternalRequestParameters.uidSO1, false);
		} else if (StringUtils.isNotBlank(casInternalRequestParameters.uidO1)) {
			appendQueryParam(url, SHA1UDID, casInternalRequestParameters.uidO1,
					false);
		}
		if (StringUtils.isNotBlank(casInternalRequestParameters.uidMd5)) {
			appendQueryParam(url, MD5UDID, casInternalRequestParameters.uidMd5,
					false);
		} else if (StringUtils.isNotBlank(casInternalRequestParameters.uid)) {
			appendQueryParam(url, MD5UDID, casInternalRequestParameters.uid,
					false);
		}
	...
	public void parseResponse(final String response,
		...
		if (null == response || status.code() != 200
			...
			try {
				Request request = (Request) jaxbUnmarshaller
