...
public class GenericAdapter extends BaseAdNetworkImpl {
  ...
  public boolean isBeaconUrlRequired() {
    return config.getString(advertiserName.concat(MacrosAndStrings.IS_BEACON_REQUIRED)).equals(MacrosAndStrings.TRUE);
  }

  @Override
  public boolean isClickUrlRequired() {
    return config.getString(advertiserName.concat(MacrosAndStrings.IS_CLICK_REQUIRED)).equals(MacrosAndStrings.TRUE);
  }
    ...
    if (responseFormat.equals(MacrosAndStrings.JSON)) {
      if (status.code() != 200 || StringUtils.isBlank(response)) {
        ...
        try {
          ...
          if (responseInJson.getString(config.getString(advertiserName + MacrosAndStrings.RESPONSE_STATUS)).equals(
              config.getString(advertiserName + MacrosAndStrings.STATUS_NO_AD))) {
            statusCode = 500;
            responseContent = "";
          } else {
            statusCode = status.code();
            ...
            final String impressionUrl =
                responseInJson
                    .getString(config.getString(advertiserName.concat(MacrosAndStrings.IMPRESSION_URL_FIELD)));
            responseContent =
                responseWithoutImpressionUrl.replaceAll(MacrosAndStrings.HTML_ENDING, "<img src=\"" + impressionUrl
                    + "\" height=1 width=1 border=0 />" + MacrosAndStrings.HTML_ENDING);
          }
