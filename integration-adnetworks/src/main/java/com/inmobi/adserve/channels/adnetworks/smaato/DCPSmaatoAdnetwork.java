...
public class DCPSmaatoAdnetwork extends AbstractDCPAdNetworkImpl {
	private static final Logger LOG = LoggerFactory
			.getLogger(DCPSmaatoAdnetwork.class);
	...
	public DCPSmaatoAdnetwork(final Configuration config,
	...
	public boolean configureParameters() {
		...
		if (null != sasParams.getSlot()
			...
			Dimension dim = SlotSizeMapping.getDimension((sasParams.getSlot())
					.longValue());
	...
	public URI getRequestUri() throws Exception {
		...
		if (StringUtils.isNotBlank(casInternalRequestParameters.uidIFA)) {
			appendQueryParam(url, IFA, casInternalRequestParameters.uidIFA,
					false);
			appendQueryParam(url, IFA_TRACKING,
					casInternalRequestParameters.uidADT, false);
		}
		...
		if (StringUtils.isNotBlank(casInternalRequestParameters.uid)) {
			appendQueryParam(url, OPEN_UDID, casInternalRequestParameters.uid,
					false);
		}
		if (StringUtils.isNotBlank(casInternalRequestParameters.uidSO1)) {
			appendQueryParam(url, ODIN1, casInternalRequestParameters.uidSO1,
					false);
		} else if (StringUtils.isNotBlank(casInternalRequestParameters.uidO1)) {
			appendQueryParam(url, ODIN1, casInternalRequestParameters.uidO1,
					false);
		}
		...
		if (StringUtils.isNotBlank(latitude)
			...
			appendQueryParam(
					...
					LATLONG,
					getURLEncode(
							String.format(latLongFormat, latitude, longitude),
							format), false);
		}
		...
		if (null != sasParams.getPostalCode()) {
			appendQueryParam(url, ZIP, sasParams.getPostalCode().toString(),
					false);
		}
		appendQueryParam(url, KEYWORDS,
				getURLEncode(getCategories(',', true, false), format), false);
		if (width != 0) {
	...
	public void parseResponse(final String response,
		...
		if (null == response || status.code() != 200
			...
			try {
				Response smaatoResponse = (Response) jaxbUnmarshaller
						.unmarshal(new ByteArrayInputStream(response.getBytes()));

				if (!SUCCESS.equalsIgnoreCase(smaatoResponse.getStatus())
						|| smaatoResponse.getAds().getAd() == null) {
					adStatus = "NO_AD";
					statusCode = 500;
					responseContent = "";
					return;
				...
				TemplateType t = null;
				context.put(VelocityTemplateFieldConstants.PartnerClickUrl, ad
						.getAction().getTarget());
				context.put(VelocityTemplateFieldConstants.IMClickUrl, clickUrl);

				context.put(VelocityTemplateFieldConstants.PartnerBeaconUrl, ad
						.getBeacons().getBeacon());
				if (IMAGE_TYPE.equalsIgnoreCase(ad.getType())
						&& StringUtils.isNotBlank(ad.getLink())) {
					context.put(VelocityTemplateFieldConstants.PartnerImgUrl,
							ad.getLink());
					t = TemplateType.IMAGE;
				} else if (TEXT_TYPE.equalsIgnoreCase(ad.getType())
						&& StringUtils.isNotBlank(ad.getAdtext())) {
					context.put(VelocityTemplateFieldConstants.AdText,
							ad.getAdtext());
					String vmTemplate = Formatter
							.getRichTextTemplateForSlot(slot.toString());
					if (StringUtils.isEmpty(vmTemplate)) {
						t = TemplateType.PLAIN;
					} else {
						context.put(VelocityTemplateFieldConstants.Template,
								vmTemplate);
						t = TemplateType.RICH;
					}
				} else {
