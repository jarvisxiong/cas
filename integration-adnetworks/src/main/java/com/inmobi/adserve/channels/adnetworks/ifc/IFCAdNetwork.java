...
public class IFCAdNetwork extends AbstractDCPAdNetworkImpl {
  ...
  public boolean configureParameters() {
    ...
    if (null != jsonObject) {
      ...
      if (tempSdkVersion != null
          && (tempSdkVersion.toLowerCase().startsWith("i30") || tempSdkVersion.toLowerCase().startsWith("a30"))) {
        return false;
      }
    }
  ...
  protected Request getNingRequest() throws Exception {
    ...
    return new RequestBuilder("POST").setUrl(uri.toString())
        ...
        .setHeader(HttpHeaders.Names.ACCEPT, "application/json").setHeader(HttpHeaders.Names.HOST, uri.getHost())
        .setBody(body).build();
  }
