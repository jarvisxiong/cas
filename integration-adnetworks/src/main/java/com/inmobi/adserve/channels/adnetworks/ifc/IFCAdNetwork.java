...
public class IFCAdNetwork extends BaseAdNetworkImpl {
	...
	public IFCAdNetwork(final DebugLogger logger,
	...
	public boolean configureParameters() {
		...
		try {
			...
			deviceOSVersion = stringifyParam(jsonObject, "deviceOSVersion",
					false);
			...
			siteAllowBanner = getFlagParams(jsonObject, "site-allowBanner",
					false);
		...
		try {
			...
			logger.info("IFC Mandatory Parameter missing: handset");
			return false;
		}
		try {
			carrier = getCarrierString(jsonObject.getJSONArray("carrier"));
		} catch (JSONException e) {
			logger.info("IFC Mandatory Parameter missing: carrier");
			return false;
		}
		...
		try {
			...
			if (sdkVersion != null
					&& (sdkVersion.toLowerCase().startsWith("i30") || sdkVersion
							.toLowerCase().startsWith("a30"))) {
				return false;
			}
		} catch (JSONException e) {
			logger.info("Error while parsing 'sdk-version'");
		}
		...
		if (!isEmpty(sasParams.getGender())
				&& (sasParams.getGender().equalsIgnoreCase("m") || sasParams
						.getGender().equalsIgnoreCase("male"))) {
			gender = "male";
		} else if (!isEmpty(sasParams.getGender())
				&& (sasParams.getGender().equalsIgnoreCase("f") || sasParams
		...
		if (null != sasParams.getSiteId()) {
			...
			logger.info("IFC Mandatory Parameter missing: SiteName");
			return false;
		}
	...
	private Boolean getFlagParams(final JSONObject jsonObject,
			final String field, final boolean isMandatory) throws JSONException {
		Boolean booleanVal = null;
		try {
			booleanVal = jsonObject.getBoolean(field);
		} catch (JSONException e) {
			if (isMandatory) {
				logger.info("IFC Mandatory Parameter missing:", field);
				throw new JSONException("IFC Mandatory Parameter missing:"
						+ field);
			}
		}
		return booleanVal;
	...
	private String stringifyParam(final JSONObject jObject, final String field,
			final boolean isMandatory) throws JSONException {
		try {
	...
	public HttpRequest getHttpRequest() {
		...
		httpRequest = new DefaultHttpRequest(HttpVersion.HTTP_1_1,
				HttpMethod.POST, uri.toASCIIString());
		ChannelBuffer buffer = ChannelBuffers.copiedBuffer(getRequestBody(),
				CharsetUtil.UTF_8);
	...
	public JSONObject getLogline() {
		try {
			...
			logger.info("Error while constructing logline inside ifc adapter");
			return null;
		}
	}
	...
	private String getRequestBody() {
		...
		jsonObject.addProperty("responseTime",
				String.valueOf(config.getInt("ifc.readtimeoutMillis")));
		...
		jsonObject.addProperty("uidIDUS1",
	...
	private String getCategoryString(final List<Long> categories) {
		if (null != categories) {
			...
			for (int i = 0; i < categories.size(); i++) {
				if (null != CategoryList.getCategory(categories.get(i)
						.intValue())) {
					categoryString.append(
							CategoryList.getCategory(categories.get(i)
									.intValue())).append(',');
				}
			}
	...
	private String getHandsetString(final JSONArray handset)
	...
	private String getCarrierString(final JSONArray carrier)
			throws JSONException {
		if (null != carrier) {
	...
	public void parseResponse(final String response,
		...
		if (null == response
				|| (null != response && (status.getCode() != 200
						|| response.startsWith("<!--") || response.trim()
						.isEmpty()))) {
