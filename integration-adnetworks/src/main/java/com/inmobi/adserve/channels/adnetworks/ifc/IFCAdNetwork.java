...
public class IFCAdNetwork extends BaseAdNetworkImpl {
    ...
    private static final String name        = "ifc";
    ...
    public boolean configureParameters() {
        ...
        if (null == clickUrl) {
            logger.info("IFC Mandatory Parameter missing: clickUrl");
            return false;
        }
        return true;
    }
    ...
    private String getRequestBody() {
        ...
        jsonObject.addProperty("clurl", clickUrl);
        ...
        jsonObject.addProperty("clickURL", clickUrl);
    ...
    public void parseResponse(final String response, final HttpResponseStatus status) {
        if (null == response
            ...
            /*
             * if(!StringUtils.isEmpty(responseContent) && !StringUtils.isEmpty(sasParams.getImaiBaseUrl())) { String
             * iMAIBaseUrlFix =
             * "<head><meta name=\"viewport\" content=\"width=device-width, height=device-height,user-scalable=0, minimum-scale=1.0, maximum-scale=1.0\"/><base href=\"%s\"></base>"
             * ; iMAIBaseUrlFix = String.format(iMAIBaseUrlFix, sasParams.getImaiBaseUrl()); responseContent =
             * responseContent.replaceFirst("<head>", iMAIBaseUrlFix); }
             */
            statusCode = status.getCode();
            adStatus = "AD";
        }
