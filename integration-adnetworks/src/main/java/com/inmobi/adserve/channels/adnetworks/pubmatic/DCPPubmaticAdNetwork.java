...
public class DCPPubmaticAdNetwork extends AbstractDCPAdNetworkImpl {
	private static final Logger LOG = LoggerFactory
			.getLogger(DCPPubmaticAdNetwork.class);
	...
	public boolean configureParameters() {
		...
		if (null != slotSizeMapEntity) {
			...
			try {
				final JSONObject additionalParams = entity
						.getAdditionalParams();
				...
				LOG.error(
						"AdId is not configured for the segment:{}, exception raised {}",
		...
		if (!"wap".equalsIgnoreCase(sasParams.getSource())
				&& StringUtils.isBlank(udid)) { // deviceid mandatory for
												// App traffic
			LOG.debug("mandate parameters missing for pubmatic, so returning from adapter");
			LOG.info("Configure parameters inside pubmatic returned false");
			return false;
	...
	public String getRequestParams() {
		...
		params.append("&kadwidth=").append(width).append("&kadheight=")
				.append(height);
		params.append("&pageURL=").append(blindedSiteId);
		params.append("&keywords=").append(
				getURLEncode(getCategories(','), format));
		final SimpleDateFormat dfm = new SimpleDateFormat(DATE_FORMAT);
		params.append("&kltstamp=").append(
				getURLEncode(dfm.format(Calendar.getInstance().getTime()),
						format));
	...
	public void parseResponse(final String response,
		...
		if (null == response || status.code() != 200
			...
			try {
				...
				adStatus = "NO_AD";
				LOG.info("Error parsing response {} from pubmatic: {}",
						response, exception);
	...
	private void setDeviceIdandType() {
		if (sasParams.getOsId() == HandSetOS.iOS.getValue()) {
			if (StringUtils
					.isNotBlank(casInternalRequestParameters.getUidIFA())) {
