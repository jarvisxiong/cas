...
public abstract class BaseAdNetworkImpl implements AdNetworkInterface {
      ...
      getAsyncHttpClient().executeRequest(ningRequest, new AsyncCompletionHandler() {
        ...
        public Response onCompleted(final Response response) throws Exception {
          ...
          if (!isRequestCompleted()) {
            ...
            final HttpResponseStatus httpResponseStatus = HttpResponseStatus.valueOf(response.getStatusCode());
  ...
  protected Request getNingRequest() throws Exception {
    ...
    return new RequestBuilder().setUrl(uri.toString())
        ...
        .setHeader("X-Forwarded-For", sasParams.getRemoteHostIp()).setHeader(HttpHeaders.Names.HOST, uri.getHost())
        .build();
  }
      ...
      for (int index = 0; index < sasParams.getCategories().size(); index++) {
        ...
        for (int i = 0; i < segmentCategories.length; i++) {
          if (cat == segmentCategories[i]) {
            if (isIABCategory) {
              return getValueFromListAsString(IAB_CATEGORY_MAP.getIABCategories(segmentCategories[i]), seperator);
  ...
  protected String getURLEncode(final String param, final String format) {
    ...
    if (StringUtils.isNotBlank(param)) {
      try {
        ...
        LOG.debug("Error during decode in getURLEncode() for {} for string {}, exception raised {}", getName(), param,
            uee);
      }
      try {
        encodedString = URLEncoder.encode(decoded.trim(), format);
      } catch (final UnsupportedEncodingException e) {
        LOG.debug("Error during encode in getURLEncode() for {} for string {}, exception raised {}", getName(), param,
            e);
      }
    }
