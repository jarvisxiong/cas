...
public class SiteAerospikeFeedbackRepository {
  ...
  /**
   * Method to get SegmentAdGroupFeedbackEntity for the request site and segment combination. Looks first in cache with
   * a configurable refresh time if hits within the refresh time , returns the hit entity otherwise makes a call to
   * aerospike to load the fresh data while returning the stale entity for the current request.
   * 
  ...
  public SegmentAdGroupFeedbackEntity query(final String siteId, final Integer segmentId) {
    ...
    LOG.debug("Returning default/old siteFeedback entity and fetching new data from aerospike for siteId: {}", siteId);
  ...
  class CacheUpdater implements Runnable {
        ...
        for (final Map.Entry<String, Object> binValuePair : record.bins.entrySet()) {
          ...
          if (bin.startsWith(DataCenter.ALL.toString())) {
            ...
            try {
              ...
              LOG.debug("Error in deserializing thrift for global feedback for segment {} {}", segmentId, exception);
              globalFeedback = null;
            }
            ...
            try {
              ...
              LOG.debug("Error in deserializing thrift for rct feedback for segment {} {}", segmentId, exception);
              rctFeedback = null;
            }
            ...
            try {
              ...
              LOG.debug("Error in deserializing thrift for local feedback for segment {} {}", segmentId, exception);
              coloFeedback = null;
            }
    ...
    SegmentAdGroupFeedbackEntity buildSiteFeedbackEntity(final SiteFeedback globalFeedback,
      ...
      for (final Map.Entry<String, ChannelSegmentFeedbackEntity.Builder> entry : adGroupFeedbackBuilderMap.entrySet()) {
    ...
    private ChannelSegmentFeedbackEntity.Builder buildChannelSegmentFeedbackEntityBuilder(
      ...
      for (final Map.Entry<String, Feedback> entry : adGroupFeedback.getDailyFeedback().entrySet()) {
        ...
        long age;
        try {
          age = (date.getTime() - dateFormat.parse(entry.getKey().split(" ")[0]).getTime()) / (1000 * 60 * 60 * 24);
